...
void spaces1()
{
    for (int i = 0; i < 4; i++)
        cout << endl;
    cout << "\t\t\t\t\t ROHIT BANK LIMITED (RBL) " << endl;
}
void spaces2()
{
    for (int i = 0; i < 2; i++)
...
void requestLoan(account *accounts, int &noOfAccounts, account &currentUser, char login, bool flag)
{
    ...
    if (currentUser.getTotalBalance() >= 500)
    {
        ...
        //Sleep(3 * 1000);
        system("cls");
        cout << "\t\t\t You request is approved ,You got $" << amount << " of loan from Rohit Bank Limited (RBL) " << endl;
...
================================================================================================*/
    ...
    //Account-1
    accounts[0].setName("Rohit Goyal");
    ...
    //Account-2
    accounts[1].setName("Gautam Gupta");
    ...
    //Account-3
    accounts[2].setName("Dheeraj Garg");
